...
# CSS reaches a resource through a function - url(), image-set(), src(),
# image() - and through an at-rule. The functions a drawing calls are few
# (the generated previews use two); everything else is refused rather than
# searched for by spelling, and no at-rule is needed at all.
CSS_FUNCTIONS = frozenset(
    {"not", "rgb", "rgba", "hsl", "hsla", "translate", "rotate", "scale", "matrix"}
)
CSS_FUNCTION_CALL = re.compile(r"([A-Za-z-]+)\s*\(")
...
def _css_is_static(css: str) -> bool:
    ...
    if CSS_ESCAPE.search(canonical) or "@" in canonical:
        return False
    calls = CSS_FUNCTION_CALL.findall(canonical)
    if any(name.lower() not in CSS_FUNCTIONS for name in calls):
        return False
    return not LOADS_OR_RUNS.search(canonical)


def _accepted_preview(source: Path) -> str | None:
    """The preview's text when it exists, reads as text and is static; else None."""
    if not source.exists():
        ...
        return None
    try:
        picture = source.read_text(encoding="utf-8")
        ...
        _LOGGER.error(
            "Preview %s cannot be read and is not copied: %s", source.name, err
        )
        return None
    if not is_static_picture(picture):
        _LOGGER.error(
            "Preview %s carries script or links to the web and is not copied "
            "under www/, where it would run in Home Assistant's origin. Redraw "
            "it with compile_kennfeld.py",
            source.name,
        )
        return None
    return picture
...
class PowerMap:
    ...
    def _copy_powermap_plot(self, json_filepath: Path | None, www_dir: Path) -> None:
        ...
        try:
            ...
            destination.write_text(picture, encoding="utf-8")
            _LOGGER.info("Updated dashboard power map image: %s", destination.name)
        except OSError as err:
            _LOGGER.error(
                "Failed to publish the power map image under www/local: %s", err
            )
